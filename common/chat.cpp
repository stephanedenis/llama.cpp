...
static common_chat_params common_chat_params_init_muse_glimmer(const common_chat_template &          tmpl,
    ...
    auto parser = build_chat_peg_parser([&](common_chat_peg_builder & p) {
        ...
        // A final answer ends at <|eot|>, but the model may also answer the user
        // and then act, closing the answer with <|eom|> before the tool turn.
        // Stopping on either is what keeps "inform then act" from swallowing the
        ...
        if (has_tools && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE) {
            ...
            // "Inform then act": the answer is closed with <|eom|> and the call
            // follows in its own <|start|>assistant turn.
