...
// This fork predates upstream's common/json.h. nlohmann::ordered_json is the same ordered JSON
// value type the rest of common/ uses (see common/chat.h), so the port needs only this alias
// and one call-site change: common_json::make(s) -> common_json(s). Both build a JSON string
// value, which is what json_text() below relies on to quote field names and enum values.
using common_json = nlohmann::ordered_json;
